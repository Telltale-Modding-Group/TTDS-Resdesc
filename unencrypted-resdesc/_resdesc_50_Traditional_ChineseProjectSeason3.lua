local set = {}
set.name = "Traditional_ChineseProjectSeason3"
set.setName = "Traditional_ChineseProjectSeason3"
set.descriptionFilenameOverride = ""
set.logicalName = "<Traditional_ChineseProjectSeason3>"
set.logicalDestination = "<>"
set.priority = 1500
set.localDir = _currentDirectory
set.enableMode = "localization"
set.version = "trunk"
set.descriptionPriority = 0
set.gameDataName = "Traditional_ChineseProjectSeason3 Game Data"
set.gameDataPriority = 0
set.gameDataEnableMode = "constant"
set.localDirIncludeBase = true
set.localDirRecurse = false
set.localDirIncludeOnly = nil
set.localDirExclude = 
{
    "_dev/"
}
set.gameDataArchives = 
{
    _currentDirectory .. "WDC_pc_Traditional_ChineseProjectSeason3_compressed.ttarch2"
}
RegisterSetDescription(set)
