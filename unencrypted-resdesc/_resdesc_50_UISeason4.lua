local set = {}
set.name = "UISeason4"
set.setName = "UISeason4"
set.descriptionFilenameOverride = ""
set.logicalName = "<UISeason4>"
set.logicalDestination = "<>"
set.priority = 33
set.localDir = _currentDirectory
set.enableMode = "bootable"
set.version = "trunk"
set.descriptionPriority = 0
set.gameDataName = "UISeason4 Game Data"
set.gameDataPriority = 0
set.gameDataEnableMode = "constant"
set.localDirIncludeBase = true
set.localDirRecurse = false
set.localDirIncludeOnly = nil
set.localDirExclude = 
{
    "Packaging/",
    "_dev/"
}
set.gameDataArchives = 
{
    _currentDirectory .. "WDC_pc_UISeason4_anichore.ttarch2",
    _currentDirectory .. "WDC_pc_UISeason4_data.ttarch2",
    _currentDirectory .. "WDC_pc_UISeason4_txmesh.ttarch2"
}
RegisterSetDescription(set)
